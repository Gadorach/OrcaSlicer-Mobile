...
bool has_brim(const Layer* layer, const Params& params){
    return
        ...
        && (params.brim_width > 0.0 || params.brim_type == BrimType::btAutoBrim);
}
...
Polygons get_brim(const ExPolygon& slice_polygon, const BrimType brim_type, const float brim_width) {
    ...
    ExPolygons brim;
    if (brim_type == BrimType::btOuterAndInner || brim_type == BrimType::btOuterOnly || brim_type == BrimType::btAutoBrim) {
